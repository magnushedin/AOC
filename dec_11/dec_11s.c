...
int grid_power_level(int x, int y, int array[301][301]) {
    int power = 0;
...
int main(int argc, char* argv[])
{
    ...
    for (int x=1;x<=298;x++) {
        for (int y=1;y<=298;y++) {
            grid_power = grid_power_level(x,y,array);
    ...
    printf("Largest power_level at (%d,%d) with value %d\n", x_max, y_max, max_grid_power);
